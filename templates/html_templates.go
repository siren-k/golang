...
// HTMLDifferences 함수는 html/template과 text/template 간의 차이점을 강조한다.
func HTMLDifferences() error {
	...
	// html/template은 자바스크립트 주입과 같은 안전하지 않은 동작을 자동 회피(탐출, escape) 처리한다.
	// 이는 상황에 따라 인식되며 변수가 렌더링되는 위치에 따라 다르게 동작한다.
